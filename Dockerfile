...
RUN echo 'install.packages("devtools", repos="http://mirror.las.iastate.edu/CRAN/")' >> /tmp/packages.R
RUN echo 'devtools::install_github("pachterlab/sleuth")' >> /tmp/packages.R
RUN echo 'library("sleuth")' >> /tmp/packages.R
RUN echo 'install.packages("ggplot2", repos="http://mirror.las.iastate.edu/CRAN/")' >> /tmp/packages.R
RUN echo 'library("ggplot2")' >> /tmp/packages.R
RUN echo 'install.packages("plyr", repos="http://mirror.las.iastate.edu/CRAN/")' >> /tmp/packages.R
RUN echo 'library("plyr")' >> /tmp/packages.R
RUN echo 'install.packages("RColorBrewer", repos="http://mirror.las.iastate.edu/CRAN/")' >> /tmp/packages.R
RUN echo 'library("RColorBrewer")' >> /tmp/packages.R
RUN echo 'install.packages("stringr", repos="http://mirror.las.iastate.edu/CRAN/")' >> /tmp/packages.R
